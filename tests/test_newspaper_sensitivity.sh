...
EOF
python3 tools/build_newspaper.py --date 2026-07-08 >/dev/null
D=newspaper/drafts/2026-07-08.md
...
if python3 tools/build_newspaper.py --date 2026-07-08 --publish 2>/dev/null; then
  echo "published without publisher_verdict!"; exit 1; fi
sed -i 's/publisher_verdict: pending.*/publisher_verdict: approved/' "$D"
python3 tools/build_newspaper.py --date 2026-07-08 --publish >/dev/null
[ -f newspaper/editions/2026-07-08.md ] || { echo "publish failed"; exit 1; }
echo OK
